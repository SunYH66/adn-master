...
from ..utils import print_model, backprop_on, backprop_off#, get_device


class ADNTrain(BaseTrain):
    def __init__(self, learn_opts, loss_opts, opts, g_type, d_type, **model_opts):
        super(ADNTrain, self).__init__(learn_opts, loss_opts, opts)
        ...
        self.model_dl = add_gan_loss(self._get_trainer(model_dict, d_type), opts) # discriminator for low quality image (with artifact)
        self.model_dh = add_gan_loss(self._get_trainer(model_dict, d_type), opts) # discriminator for high quality image (without artifact)
        ...
        self.model_g._criterion["clh"] = self._get_criterion(loss_dict, self.wgts["clh"], "clh_")
        self.model_g._criterion["slh"] = self._get_criterion(loss_dict, self.wgts["slh"], "slh_")
        self.model_g._criterion["chl"] = self._get_criterion(loss_dict, self.wgts["chl"], "chl_")
        self.model_g._criterion["shl"] = self._get_criterion(loss_dict, self.wgts["shl"], "shl_")
    ...
    def optimize(self, img_low, img_high):
        ...
        if self._nonzero_weight("gl", "lh", "ll"):
            ...
            for i in range(len(self.s_h)):
                for j in range(len(self.s_h[i].size())):
                    dim[i, j] = self.s_h[i].size()[j]
            self.s_h_new = torch.zeros((len(self.s_h), int(np.max(dim[:, 0])), int(np.max(dim[:, 1])),
                                int(np.max(dim[:, 2])), int(np.max(dim[:, 3]))))
            self.s_l_h_new = torch.zeros((len(self.s_h), int(np.max(dim[:, 0])), int(np.max(dim[:, 1])),
                                int(np.max(dim[:, 2])), int(np.max(dim[:, 3]))))
            for i in range(len(self.s_h)):
                self.s_h_new[i, :self.s_h[i].size()[0], :self.s_h[i].size()[1], :self.s_h[i].size()[2], :self.s_h[i].size()[3]] = self.s_h[i]
                self.s_l_h_new[i, :self.s_h[i].size()[0], :self.s_h[i].size()[1], :self.s_h[i].size()[2], :self.s_h[i].size()[3]] = self.s_l_h[i]
            self.model_g._criterion["slh"](self.s_h_new, self.s_l_h_new)
        ...
        if self._nonzero_weight("gh", "hh"):
            ...
            for i in range(len(self.s_l)):
                for j in range(len(self.s_l[i].size())):
                    dim[i, j] = self.s_l[i].size()[j]
            self.s_l_new = torch.zeros((len(self.s_l), int(np.max(dim[:, 0])), int(np.max(dim[:, 1])),
                                int(np.max(dim[:, 2])), int(np.max(dim[:, 3]))))
            self.s_h_l_new = torch.zeros((len(self.s_l), int(np.max(dim[:, 0])), int(np.max(dim[:, 1])),
                                int(np.max(dim[:, 2])), int(np.max(dim[:, 3]))))
            for i in range(len(self.s_h)):
                self.s_l_new[i, :self.s_h[i].size()[0], :self.s_h[i].size()[1], :self.s_h[i].size()[2], :self.s_h[i].size()[3]] = self.s_l[i]
                self.s_h_l_new[i, :self.s_h[i].size()[0], :self.s_h[i].size()[1], :self.s_h[i].size()[2], :self.s_h[i].size()[3]] = self.s_h_l[i]
            self.model_g._criterion["shl"](self.s_l_new, self.s_h_l_new)
        ...
        if self._nonzero_weight("hlh"):
            self.pred_hlh = self.model_g.forward_lh(self.pred_hl, self.pred_lh)
        ...
        if self._nonzero_weight("art"):
            ...
            a = ll - self.pred_ll
            b = self.pred_lhl - ll
            self.model_g._criterion["art"](a, b)
        self.model_g._update()
        self.model_dl._update()
        self.model_dh._update()

        # self.model_g._clear()
        # self.pred_ll, self.pred_hh, self.pred_lh, self.pred_hl = self.model_g.forward_yh_test(self.img_low, self.img_high)
        # self.model_dl._clear()
        # self.model_g._criterion["ll"](self.pred_ll, self.img_low)
        # self.model_g._criterion["gl"](self.pred_lh, self.img_high)
        # self.model_dl._update()
